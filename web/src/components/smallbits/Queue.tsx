            ...
            props.player.queue.map((song) =>
                <div style={{ display: "flex", flexDirection: "column" }} className="queue-inner-wrapper" ref={queueInnerRef}>
                    <span className="queue-song-name">
                        {song.file.split(`${props.state.userData?.username}/`)[1].replace(".mp3", "")}
                    </span>
                    <span style={{ fontSize: "12px" }} className="queue-remove-song" onClick={() => props.player.removeFromQueue(song.id)}>remove?</span>
                </div>
            )
        ) : (
        ...
        <>
            {props.state.isQueueVisible ? (
                <div className={`queue-wrapper`}>
                    ...
                    <div className={`queue-inner-wrapper ${props.state.graphicalMode ? "" : "margin-fix-other-ui"}`}>
                        {showQueue}
                    </div>
                </div> 
            ) : null}
