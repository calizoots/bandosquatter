...
export let profileMenuButtonStyle: React.JSX.CSSProperties = {
...
export let GraphicalUI: React.FC<GraphicalUIProps> = props => {
    ...
    let filteredGraphicalData = props.state.data?.filter(
        file => file.title.toLowerCase().includes(props.state.searchQuery.toLowerCase()) || file.artist.toLowerCase().includes(props.state.searchQuery.toLowerCase())
    );
        ...
        <div className="music-grid-wrapper" key={file.title}>
            <div className="music-content-wrapper">
                ...
                {props.state.isUserAddingSongToPlaylist.isit ? (
                    ...
                    <>
                        <button className="play-button" onClick={() => props.player.bang(file)}>
                            play
                        </button>
                        <button className="queue-button" onClick={() => props.player.addToQueue(file)}>
                            add to queue
                        </button>
                    </>
                )}
                ...
                <div>
                    {state.isUserAddingSongToPlaylist.isit ? (
                        <div className="user-adding-song-go-back-wrapper">
                            <span
                                className="user-adding-song-go-back"
                                onClick={() => {
                                    updateState('isUserAddingSongToPlaylist', {
                                        isit: false,
                                        playlist: undefined,
                                    });
                    ...
                    {state.isSearchBarVisible ? (
                        <div className="search-bar">
                            <input type="text" className="search-bar-input" placeholder="search..." value={state.searchQuery} onInput={e => updateState('searchQuery', (e.target as HTMLInputElement).value)} />
                        </div>
                    ) : null}
                    <Queue player={props.player} state={state} />
                    <Profile
                        profilePicture={props.state.userData.profilePicture}
                    ...
                    <div className="music-wrapper">{showGraphicalData}</div>
                </div>
            ) : (
                <Loading />
            )}
