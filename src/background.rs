...
use nix::unistd::Pid;
use chrono::Local;
use tokio::{runtime, net::TcpListener};
use r2d2_sqlite::SqliteConnectionManager;
use axum::serve;

use crate::log;
use crate::database::{BSDatabase, LocalStorageManager};
use crate::server::create_api_routes;
use crate::server::create_app_routes;
use crate::server::AppState;
use crate::utils::{AnyError, LogLevel};
...
fn child_work() {
    let mng = SqliteConnectionManager::file(DB_PATH.as_path());

    let mut db = match BSDatabase::new(mng) {
        Ok(db) => db,
        Err(e) => {
            log!(LogLevel::Err, "make db err -> {}", e);
            return;
        }
    };

    if let Err(e) = db.get_migrations() {
        log!(LogLevel::Err, "db get_migrations err -> {}", e);
        return;
    }

    if let Err(err) = db.apply_migrations() {
        log!(LogLevel::Err, "db apply_migrations err -> {}", err);
    }

    let conn = match db.pool.get() {
        Ok(conn) => conn,
        Err(e) => {
            log!(LogLevel::Err, "failed to get conn out of pool for StorageManager {} (quitting now)", e);
            exit(69);
        }
    };

    let storage = LocalStorageManager::new("/Users/crack/Bang/bandosquatter/music");

    if let Err(e) = storage.ensure_user_folders(&conn) {
        log!(LogLevel::Err, "failed to ensure user folders error -> {} (quitting now)", e);
        exit(69);
    }

    let rt = runtime::Builder::new_multi_thread() // multi-threaded is better for servers
        .enable_all()
        .build()
        .unwrap();
    ...
    rt.block_on(async {
        ...
        tokio::task::spawn_blocking(move || {
            let conn = match pool.get() {
                Ok(conn) => conn,
                Err(e) => {
                    log!(LogLevel::Err, "failed to get conn out of pool for propagate db {} (quitting now)", e);
                    exit(69);
                }
            };

            if let Err(e) = storage_clone.propagate_db(&conn) {
                log!(LogLevel::Err, "propagate db failed: {}", e);
            }
        });

        let html_app = create_app_routes();
        let api_app = create_api_routes(AppState { storage, db });

        let html_addr = SocketAddr::from(([127, 0, 0, 1], APP_PORT));
        let api_addr = SocketAddr::from(([127, 0, 0, 1], API_PORT));

        let html_server = tokio::spawn(async move {
            let listener = TcpListener::bind(&html_addr).await.unwrap();
            serve(listener, html_app.into_make_service()).await.unwrap();
        });

        let api_server = tokio::spawn(async move {
            let listener = TcpListener::bind(&api_addr).await.unwrap();
            serve(listener, api_app.into_make_service()).await.unwrap();
        });

        log!(LogLevel::Info, "html server on http://{}", html_addr);
        log!(LogLevel::Info, "api server on http://{}", api_addr);

        let check = tokio::join!(html_server, api_server);

        check.0.unwrap_or_else(|_| {
            log!(LogLevel::Err, "failed to join threads (exiting)");
            exit(69);
        });

        check.1.unwrap_or_else(|_| {
            log!(LogLevel::Err, "failed to join threads (exiting)");
            exit(69);
        });
    });
    ...
    /*
    let x_req = match UsersTable::get_by_username("hello".into(), &conn) {
        Ok(x) => x,
        Err(e) => {
            log!(LogLevel::Err, "failed to get the username err -> {}", e);
            return;
        }
    };

    let x_val = match x_req {
        Some(val) => val,
        None => {
            log!(LogLevel::Err, "username hello doesnt exist");
            return;
        }
    };
